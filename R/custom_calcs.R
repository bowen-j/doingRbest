#' A generic function used to generate a set of relevant underlying calculations for the specific analysis in question
#' 
...
#' calcs(model)
#
calcs <- function(model          = NULL,
                  predictors     = NULL,
                  outcome        = NULL,
                  ...) {
  type <- attr(model, "args")$method
  if (length(type) == 0) {
    calc <- regression.calcs(model          = model,
                             predictors     = predictors,
                             outcome        = outcome)
  } else if (type == "t_test") {
    calc <- ttest.calcs(model           = model,
                        predictors      = predictors,
                        outcome         = outcome)
  } else if (type == "anova_test") {
    calc <- anova.calcs(model          = model,
                        predictors     = predictors,
                        outcome        = outcome)
  } else {
    print("TBD")
